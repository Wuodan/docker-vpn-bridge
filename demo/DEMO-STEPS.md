...
# Update files
sed -i "s#__SERVER_PRIVATE_KEY__#${SERVER_PRIVATE_KEY}#" wg-server/wg0.conf
sed -i "s#__HOST_PUBLIC_KEY__#${HOST_PUBLIC_KEY}#" wg-server/wg0.conf
sed -i "s#__HOST_PRIVATE_KEY__#${HOST_PRIVATE_KEY}#" wg-host.conf
sed -i "s#__SERVER_PUBLIC_KEY__#${SERVER_PUBLIC_KEY}#" wg-host.conf
```
...
```bash
sudo wg-quick up ./wg-host.conf
```

5) Test:
```bash
curl http://10.100.0.1:8080
# Expected: "Hello over WireGuard"
```

6) Tear down:
```bash
sudo wg-quick down ./wg-host.conf
